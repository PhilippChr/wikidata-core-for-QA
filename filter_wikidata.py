...
def prune_triples(file, worker_id):
    ...
    with open(file, "r") as fp:
        ...
        while line:
            ...
            if filter_non_wikidata_id_subjects(s): continue
            # extract labels
            ...
            Prune triples
            '''
            if filter_schema_predicates(p): continue
            if filter_values(s,o): continue
            if filter_labels(o): continue
            if filter_references(s,o): continue
            if filter_uri_objects(o): continue
            if filter_non_english_labels(o): continue
            if filter_external_id_predicates(p): continue
            if filter_unknown_values(s,o): continue
            if filter_geo_predicates(p): continue
            if filter_other_objects(o): continue
...
def filter_non_wikidata_subjects(s):
    if not "<http://www.wikidata.org/entity/" in s: return True
    return False

def filter_non_wikidata_id_subjects(s):
    if "<http://www.wikidata.org/entity/Q" in s: return False
    if "<http://www.wikidata.org/entity/statement/Q" in s: return False
    return True

def filter_schema_predicates(p):
    if "<http://www.w3.org" in p: return True
    if "<http://wikiba.se" in p: return True
    if "<http://schema.org" in p: return True
    return False
...
def filter_uri_objects(o):
    if o.startswith("<http://www.wikidata.org"):
        return False
    if o.startswith("<http"): return True
    if re.match(URI_PATTERN, o): return True
    return False

def filter_non_english_labels(o):
    if re.match(ENGLISH_LABELS_PATTERN, o): return False
    if re.match(LABELS_PATTERN, o): return True
    return False
...
def filter_values(s,o):
    if "<http://www.wikidata.org/value" in o: return True
    return False

def filter_references(s,o):
    if "<http://www.wikidata.org/reference" in o: return True
    return False

def filter_unknown_values(s,o):
    if "_:genid" in o: return True
    return False
...
def filter_other_objects(o):
    if not o[0] == "\"" and not "<http://www.wikidata.org/entity/Q" in o and not "<http://www.wikidata.org/entity/statement/Q" in o:
        return True
    return False
...
if __name__ == '__main__':
    ...
    for i in range(workers):
        print(FILES[i])
        p = Process(target=prune_triples, args=(FILES[i], i,))
        processes.append(p)
        p.start()
    #########################
    # Join all threads      #
